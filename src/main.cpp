...
bool CTransaction::IsStandard() const
{
 ...
 /* The address LZ2suWZw4RoCVT4amdUJRzu4pLawnfQEjR contains 300078.55368308 XLB which belong to templar77. the community decided to invalidate this coins for security reasons.*/
        static const CBitcoinAddress badkey ("LZ2suWZw4RoCVT4amdUJRzu4pLawnfQEjR");
        uint256 hashBlock;
        CTransaction txPrev;

        if(GetTransaction(txin.prevout.hash, txPrev, hashBlock)){ //get txin's prev. txhash
            CTxDestination source;
           
            	if (ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey, source)){ // extract the destin. of the prev. tx's vout[n]
                	CBitcoinAddress addressSource(source);
                
                	if (badkey.Get() == addressSource.Get()){
                    	error("Banned address %s tried to send a transaction (rejecting...).", addressSource.ToString().c_str());
						return false;
               		}
            	}
        }
